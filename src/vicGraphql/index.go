...
func (h *Handler) ServeHTTP(ctx iris.Context, ownerContext context.Context) *graphql.Result {
	myContext := context.WithValue(context.Background(), "ctx", ctx)
	if ownerContext != nil {
		myContext = ownerContext
	}
	return h.ContextHandler(myContext, ctx)
}
